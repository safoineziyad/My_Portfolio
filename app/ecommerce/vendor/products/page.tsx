...
export default function VendorProductsPage() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const res = await fetch(`/ecommerce/api/vendor/products?${params}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`/ecommerce/api/vendor/products/${selectedProduct.id}`, { method: 'DELETE' });
  ...
  return (
    <div className="flex-1 overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {products.map(product => {
              ...
              const statusBadge = getStatusColor(product.status);
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center gap-2 mt-4">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg transition-colors"
                      >
                        <Edit2 size={13} /> Edit
                      </button>
                      <button
                        onClick={() => { setSelectedProduct(product); setShowDeleteConfirm(true); }}
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium text-red-600 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors"
                      >
      ...
      {showAddModal && (
        <ProductForm categories={categories} vendorId={vendorId} onClose={() => setShowAddModal(false)} onSaved={fetchProducts} product={null} />
      )}
      {showEditModal && selectedProduct && (
        <ProductForm product={selectedProduct} categories={categories} vendorId={vendorId} onClose={() => { setShowEditModal(false); setSelectedProduct(null); }} onSaved={fetchProducts} />
      )}
      {showDeleteConfirm && (
